...
def create_height_map(lines):
    ...
    for line in lines:
        ...
        for letter in line:
            if letter == 'S':
                value = 0
                start_point = [len(height_map[-1]), len(height_map) - 1]
            elif letter == 'E':
                value = 25
...
def find_the_best_road(graph_roads, start_point, end_point, m):
    start_index = start_point[1] * m + start_point[0]
    end_index = end_point[1] * m + end_point[0]
    return find_path(graph_roads, start_index, end_index)[-1]
