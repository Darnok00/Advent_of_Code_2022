...
def inspections_for_monkey(monkey, monkey_array, lcm):
    list_items = copy.deepcopy(monkey.list_items)
    monkey.list_items = []
    for item in list_items:
        item = item % lcm
        if monkey.operator == '*':
            value = (item * monkey.operation_value)
...
def find_array_numbers_lcm(array_numbers):
    actual_lcm = math.lcm(array_numbers[0], array_numbers[1])
    for i in range (2, len(array_numbers)):
        actual_lcm = math.lcm(actual_lcm, array_numbers[i])

    return actual_lcm


def inspect_all_monkey_all_round(monkey_array):
    divide_factor_array = [monkey.test_number for monkey in monkey_array]
    lcm = find_array_numbers_lcm(divide_factor_array)
    ...
    for i in range(NUMBER_ROUND):
        for j in range(n):
            ...
            inspections_for_monkey(monkey, monkey_array, lcm)
